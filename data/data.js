var siteData = {
  "site": {
    "name": "Desa Warung Menteng",
    "shortName": "WM",
    "tagline": "Desa Wisata di Kaki Gunung Salak",
    "footer": "Dibangun oleh Mahasiswa KKN."
  },
  "nav": [
    { "label": "Beranda", "href": "index.html" },
    { "label": "Tentang", "href": "tentang.html", "submenu": [
      { "label": "Profil Desa", "href": "tentang.html#profil" },
      { "label": "Perangkat Desa", "href": "tentang.html#perangkat" },
      { "label": "Struktur Organisasi", "href": "tentang.html#struktur" },
      { "label": "Anggaran Desa", "href": "tentang.html#anggaran" },
      { "label": "Layanan Desa", "href": "tentang.html#layanan" },
      { "label": "Lokasi & Kontak", "href": "tentang.html#lokasi" }
    ]},
    { "label": "Pariwisata", "href": "pariwisata.html", "submenu": [
      { "label": "Destinasi", "href": "pariwisata.html#destinasi" },
      { "label": "Kuliner", "href": "pariwisata.html#kuliner" },
      { "label": "Akomodasi", "href": "pariwisata.html#akomodasi" },
      { "label": "UMKM", "href": "pariwisata.html#umkm" }
    ]},
    { "label": "Berita", "href": "berita.html", "submenu": [
      { "label": "Berita", "href": "berita.html" },
      { "label": "Galeri", "href": "berita.html#galeri" }
    ]},
    { "label": "KKN", "href": "kkn.html" }
  ],
  "hero": {
    "badge": "Desa Wisata Bogor",
    "title": "Jelajahi <span>Desa Warung Menteng</span>",
    "description": "Nikmati keindahan alam pegunungan, kebun buah segar, dan keramahan budaya Sunda di kaki Gunung Salak.",
    "stats": [
      { "value": "587 m", "label": "Ketinggian" },
      { "value": "9+", "label": "Destinasi" },
      { "value": "30 menit", "label": "Dari Bogor" }
    ]
  },
  "profile": {
    "title": "Selamat Datang di Desa Warung Menteng",
    "description1": "Terletak di Kecamatan Cijeruk, Kabupaten Bogor, Desa Warung Menteng adalah permata tersembunyi di kaki Gunung Salak. Dengan udara sejuk, pemandangan hijau, dan masyarakat yang ramah, desa ini menawarkan pengalaman wisata alam yang autentik.",
    "description2": "Dari agrowisata Warso Farm, camping di Bukit Alesano, hingga situs sejarah Tugu Maseng \u2014 setiap sudut desa ini menyimpan cerita dan keindahan yang menunggu untuk dijelajahi.",
    "stats": [
      { "value": "2,28", "label": "Km\u00B2 Luas" },
      { "value": "8.872", "label": "Jiwa" },
      { "value": "587", "label": "Mdpl" },
      { "value": "9", "label": "Destinasi" }
    ]
  },
  "perangkat_desa": [
    {
      "nama": "H. Asep Saepudin, S.Pd.",
      "jabatan": "Kepala Desa",
      "bidang": "Pimpinan Pemerintahan Desa",
      "icon": "\uD83D\uDC64"
    },
    {
      "nama": "Dedi Mulyadi, S.Sos.",
      "jabatan": "Sekretaris Desa",
      "bidang": "Administrasi & Kesekretariatan",
      "icon": "\uD83D\uDCDD"
    },
    {
      "nama": "Ai Siti Patimah, S.E.",
      "jabatan": "Kasi Pemerintahan",
      "bidang": "Pelayanan Pemerintahan",
      "icon": "\uD83C\uDFDB\uFE0F"
    },
    {
      "nama": "Ade Komarudin",
      "jabatan": "Kasi Kesejahteraan",
      "bidang": "Kesejahteraan Rakyat",
      "icon": "\uD83E\uDD1D"
    },
    {
      "nama": "Rina Marlina, A.Md.",
      "jabatan": "Kasi Pelayanan",
      "bidang": "Pelayanan Publik",
      "icon": "\uD83E\uDDCD"
    },
    {
      "nama": "Usep Saepuloh",
      "jabatan": "Kaur Keuangan",
      "bidang": "Pengelolaan Keuangan Desa",
      "icon": "\uD83D\uDCB0"
    },
    {
      "nama": "Wawan Setiawan",
      "jabatan": "Kaur Perencanaan",
      "bidang": "Perencanaan Pembangunan",
      "icon": "\uD83D\uDCCA"
    },
    {
      "nama": "Euis Khoeriyah",
      "jabatan": "Kaur Umum",
      "bidang": "Ketatausahaan & Umum",
      "icon": "\uD83D\uDCE6"
    },
    {
      "nama": "Kepala Dusun I",
      "jabatan": "Kadus Cibalung",
      "bidang": "Wilayah Dusun Cibalung",
      "icon": "\uD83C\uDFE0"
    },
    {
      "nama": "Kepala Dusun II",
      "jabatan": "Kadus Warung Menteng",
      "bidang": "Wilayah Dusun Warung Menteng",
      "icon": "\uD83C\uDFE1"
    }
  ],
  "struktur_organisasi": [
    { "jabatan": "Badan Permusyawaratan Desa (BPD)", "nama": "Drs. H. Ahmad Suherman, M.M.", "level": 1 },
    { "jabatan": "Kepala Desa", "nama": "H. Asep Saepudin, S.Pd.", "level": 1 },
    { "jabatan": "Lembaga Pemberdayaan Masyarakat (LPM)", "nama": "Drs. Maman Suryaman", "level": 1 },
    { "jabatan": "Sekretaris Desa", "nama": "Dedi Mulyadi, S.Sos.", "level": 2 },
    { "jabatan": "Staf Sekretariat", "nama": "Petugas Administrasi", "level": 3 },
    { "jabatan": "Kasi Pemerintahan", "nama": "Ai Siti Patimah, S.E.", "level": 2 },
    { "jabatan": "Kasi Kesejahteraan", "nama": "Ade Komarudin", "level": 2 },
    { "jabatan": "Kasi Pelayanan", "nama": "Rina Marlina, A.Md.", "level": 2 },
    { "jabatan": "Kaur Keuangan", "nama": "Usep Saepuloh", "level": 2 },
    { "jabatan": "Kaur Perencanaan", "nama": "Wawan Setiawan", "level": 2 },
    { "jabatan": "Kaur Umum", "nama": "Euis Khoeriyah", "level": 2 },
    { "jabatan": "Kadus Cibalung", "nama": "Kepala Dusun I", "level": 3 },
    { "jabatan": "Kadus Warung Menteng", "nama": "Kepala Dusun II", "level": 3 }
  ],
  "anggaran_desa": [
    { "tahun": "2026", "bidang": "Penyelenggaraan Pemerintahan", "anggaran": "Rp 450.000.000", "sumber": "ADD", "keterangan": "Belanja operasional perangkat desa, ATK, rapat" },
    { "tahun": "2026", "bidang": "Pembangunan Infrastruktur", "anggaran": "Rp 650.000.000", "sumber": "DD", "keterangan": "Rabat beton jalan dusun, drainase, MCK" },
    { "tahun": "2026", "bidang": "Pemberdayaan Masyarakat", "anggaran": "Rp 280.000.000", "sumber": "ADD & BKM", "keterangan": "Pelatihan UMKM, kelompok tani, posyandu" },
    { "tahun": "2026", "bidang": "Penanggulangan Bencana", "anggaran": "Rp 120.000.000", "sumber": "BTT", "keterangan": "Siaga bencana, logistik, evakuasi" },
    { "tahun": "2026", "bidang": "Pembinaan Kemasyarakatan", "anggaran": "Rp 180.000.000", "sumber": "ADD", "keterangan": "Kegiatan keagamaan, olahraga, karang taruna, PKK" },
    { "tahun": "2026", "bidang": "Bidang Darurat & Mendesak", "anggaran": "Rp 100.000.000", "sumber": "Silpa", "keterangan": "Cadangan kegiatan tidak terduga" }
  ],
  "news": [
    {
      "id": 1,
      "date": "15 Juni 2026",
      "title": "Festival Durian Warung Menteng 2026",
      "excerpt": "Festival durian tahunan akan digelar pada akhir Juni. Cicipi durian lokal langsung dari kebun, lomba makan durian, dan bazar UMKM desa.",
      "category": "Kegiatan"
    },
    {
      "id": 2,
      "date": "1 Juni 2026",
      "title": "Pembukaan Jalur Tracking Baru",
      "excerpt": "Desa Warung Menteng meresmikan jalur tracking baru menyusuri perbukitan dengan pemandangan Gunung Salak yang spektakuler.",
      "category": "Wisata"
    },
    {
      "id": 3,
      "date": "20 Mei 2026",
      "title": "Pelatihan Digital Marketing untuk UMKM",
      "excerpt": "Mahasiswa KKN mengadakan pelatihan digital marketing bagi pelaku UMKM desa. Materi meliputi foto produk, media sosial, dan marketplace.",
      "category": "Pengumuman"
    },
    {
      "id": 4,
      "date": "10 Mei 2026",
      "title": "Warung Menteng Masuk 10 Besar Desa Wisata",
      "excerpt": "Desa Warung Menteng berhasil masuk nominasi 10 besar desa wisata terbaik tingkat Kabupaten Bogor.",
      "category": "Prestasi"
    }
  ],
  "services": [
    {
      "id": "sktm",
      "name": "Surat Keterangan Tidak Mampu (SKTM)",
      "description": "Surat keterangan untuk warga kurang mampu guna keperluan pendidikan, kesehatan, atau bantuan sosial.",
      "requirements": "KTP, KK, surat pengantar RT/RW",
      "icon": "\uD83D\uDCCB"
    },
    {
      "id": "sku",
      "name": "Surat Keterangan Usaha (SKU)",
      "description": "Surat keterangan bagi warga yang memiliki usaha mikro/kecil untuk keperluan perizinan atau pengajuan modal.",
      "requirements": "KTP, KK, foto usaha",
      "icon": "\uD83C\uDFEA"
    },
    {
      "id": "skd",
      "name": "Surat Keterangan Domisili",
      "description": "Surat keterangan tempat tinggal untuk warga yang berdomisili di desa namun berbeda dengan KTP.",
      "requirements": "KTP, KK, surat pengantar RT/RW",
      "icon": "\uD83C\uDFE0"
    },
    {
      "id": "skb",
      "name": "Surat Keterangan Belum Menikah",
      "description": "Surat keterangan status belum menikah untuk keperluan administrasi atau pernikahan.",
      "requirements": "KTP, KK, surat pengantar RT/RW",
      "icon": "\uD83D\uDCC4"
    }
  ],
  "umkm": [
    {
      "id": 1,
      "name": "Madu Hutan Gunung Salak",
      "description": "Madu murni dari hutan sekitar Gunung Salak. Dipanen langsung oleh petani madu lokal. Kaya manfaat untuk kesehatan.",
      "owner": "Bapak Asep",
      "contact": "628123456701",
      "icon": "\uD83C\uDF6F"
    },
    {
      "id": 2,
      "name": "Kerajinan Anyaman Bambu",
      "description": "Bakul, tampah, dan berbagai kerajinan anyaman bambu buatan tangan warga desa. Ramah lingkungan dan bernilai seni tinggi.",
      "owner": "Ibu Siti",
      "contact": "628123456702",
      "icon": "\uD83E\uDDFA"
    },
    {
      "id": 3,
      "name": "Kopi Arabika Cijeruk",
      "description": "Kopi arabika khas Cijeruk dengan cita rasa kuat dan aroma khas. Ditanam di lereng Gunung Salak pada ketinggian 800+ mdpl.",
      "owner": "Kelompok Tani Mekar Sari",
      "contact": "628123456703",
      "icon": "\u2615"
    },
    {
      "id": 4,
      "name": "Keripik Singkong Pedas",
      "description": "Keripik singkong renyah dengan bumbu pedas khas. Oleh-oleh wajib dari Desa Warung Menteng.",
      "owner": "Ibu Dedeh",
      "contact": "628123456704",
      "icon": "\uD83C\uDF60"
    },
    {
      "id": 5,
      "name": "Tanaman Hias & Bibit Buah",
      "description": "Berbagai tanaman hias dan bibit buah-buahan berkualitas. Melayani pengiriman ke berbagai kota.",
      "owner": "Bapak Usep",
      "contact": "628123456705",
      "icon": "\uD83C\uDF3F"
    },
    {
      "id": 6,
      "name": "Gula Aren Murni",
      "description": "Gula aren cetak dan cair dari nira pohon aren pilihan. Manis alami tanpa bahan pengawet.",
      "owner": "Bapak Dedi",
      "contact": "628123456706",
      "icon": "\uD83C\uDF6C"
    }
  ],
  "destinations": [
    { "id": 1, "name": "Warso Farm", "tag": "Agrowisata", "description": "Kebun buah dan sayur organik. Petik langsung durian, rambutan, dan sayuran segar. Cocok untuk liburan keluarga & edukasi anak.", "icon": "\uD83C\uDF3F" },
    { "id": 2, "name": "Bukit Alesano", "tag": "Camping", "description": "Camping ground dengan panorama Gunung Salak yang spektakuler. Spot favorit untuk menikmati matahari terbit dan langit malam.", "icon": "\uD83C\uDFD5\uFE0F" },
    { "id": 3, "name": "Tugu Maseng", "tag": "Sejarah", "description": "Monumen perjuangan rakyat melawan penjajahan Belanda. Berasal dari kata 'Maas\u00E9ng' yang berarti menyerang. Simbol semangat juang.", "icon": "\uD83D\uDDFF" },
    { "id": 4, "name": "Happy Land Cibalung", "tag": "Rekreasi", "description": "Taman rekreasi keluarga dengan wahana permainan, area piknik, dan pemandangan alam yang indah.", "icon": "\uD83C\uDFA1" },
    { "id": 5, "name": "Lembah Salak Camp", "tag": "Alam", "description": "Camping ground di lembah asri dengan fasilitas outbound. Cocok untuk gathering, keluarga, dan komunitas.", "icon": "\uD83C\uDF3F" },
    { "id": 6, "name": "Bukit Panorama", "tag": "Alam", "description": "Spot camping dengan view 360\u00B0 perbukitan. Tempat terbaik untuk hunting foto alam dan sunset.", "icon": "\u26F0\uFE0F" }
  ],
  "culinary": [
    { "id": 1, "name": "Nasi Liwet Sunda", "tag": "Makanan", "description": "Nasi gurih dengan lauk ayam goreng, tahu tempe, sambal terasi, dan lalapan segar. Hidangan khas yang menggoyang lidah.", "icon": "\uD83C\uDF5A" },
    { "id": 2, "name": "Sate Maranggi", "tag": "Makanan", "description": "Sate daging sapi khas Sunda dengan bumbu kecap rempah, dipanggang dengan arang. Disajikan dengan ketan dan sambal.", "icon": "\uD83C\uDF62" },
    { "id": 3, "name": "Durian Lokal", "tag": "Oleh-oleh", "description": "Durian khas Cijeruk dengan daging buah tebal dan rasa manis legit. Bisa dibeli langsung dari kebun atau agrowisata.", "icon": "\uD83C\uDF48" }
  ],
  "accommodation": [
    { "id": 1, "name": "Homestay Warga", "tag": "Homestay", "description": "Rasakan keramahan warga desa dengan menginap di homestay lokal. Kamar sederhana, bersih, dan harga bersahabat.", "icon": "\uD83C\uDFE0" },
    { "id": 2, "name": "Bukit Alesano Camp", "tag": "Camping", "description": "Area camping luas dengan fasilitas toilet, area api unggun, dan pemandangan Gunung Salak. Cocok untuk kelompok.", "icon": "\uD83C\uDFD5\uFE0F" },
    { "id": 3, "name": "Alang-Alang Nature Camp", "tag": "Glamping", "description": "Pengalaman glamping dengan tenda nyaman di tengah alam. Fasilitas lengkap untuk pengalaman menginap yang tak terlupakan.", "icon": "\uD83C\uDF3F" }
  ],
  "gallery": [
    { "id": 1, "label": "Perbukitan Warung Menteng", "icon": "\u26F0\uFE0F", "span": 2 },
    { "id": 2, "label": "Tugu Maseng", "icon": "\uD83D\uDDFF", "span": 1 },
    { "id": 3, "label": "Warso Farm", "icon": "\uD83C\uDF3F", "span": 1 },
    { "id": 4, "label": "Bukit Alesano", "icon": "\uD83C\uDFD5\uFE0F", "span": 1 },
    { "id": 5, "label": "Hamparan Sawah", "icon": "\uD83C\uDF3E", "span": 1 },
    { "id": 6, "label": "Bukit Panorama", "icon": "\uD83C\uDF05", "span": 2 },
    { "id": 7, "label": "Kegiatan Warga", "icon": "\uD83D\uDC68\u200D\uD83C\uDF3E", "span": 1 },
    { "id": 8, "label": "Suasana Desa", "icon": "\uD83C\uDFD8\uFE0F", "span": 1 },
    { "id": 9, "label": "Kuliner Khas", "icon": "\uD83C\uDF5A", "span": 1 }
  ],
  "testimonials": [
    { "id": 1, "name": "Andi Pratama", "from": "Wisatawan dari Jakarta", "initial": "A", "quote": "Suasananya masih asri banget, udaranya sejuk. Apalagi camping di Bukit Alesano, pemandangan Gunung Salaknya juara!", "stars": 5 },
    { "id": 2, "name": "Siti Nurhaliza", "from": "Wisatawan dari Bekasi", "initial": "S", "quote": "Warso Farm recommended banget buat keluarga. Anak-anak bisa belajar berkebun dan petik buah langsung. Seru dan edukatif!", "stars": 5 },
    { "id": 3, "name": "Dewi Lestari", "from": "Food Blogger", "initial": "D", "quote": "Kulinernya enak-enak! Nasi liwet dan sate marangginya juara. Jangan lupa cobain durian lokal yang manis banget.", "stars": 5 }
  ],
  "events": [
    { "id": 1, "date": "Januari 2026", "title": "Festival Durian Warung Menteng", "description": "Festival tahunan durian lokal. Cicipi dan beli durian segar langsung dari petani. Ada juga lomba makan durian dan bazar UMKM." },
    { "id": 2, "date": "Maret 2026", "title": "Hajat Lembur \u2014 Syukuran Desa", "description": "Acara tradisional syukuran desa dengan pertunjukan wayang golek, tarian Sunda, dan pagelaran seni budaya." },
    { "id": 3, "date": "Juni 2026", "title": "Lomba 17 Agustusan", "description": "Perlombaan tradisional meriahkan HUT Kemerdekaan RI. Seluruh warga berpartisipasi dalam berbagai cabang lomba." },
    { "id": 4, "date": "September 2026", "title": "Festival Agrowisata Cijeruk", "description": "Festival agrowisata tingkat kecamatan. Menampilkan hasil bumi, produk UMKM, dan atraksi wisata dari seluruh desa." }
  ],
  "contact": {
    "address": "Kantor Desa Warung Menteng, Kec. Cijeruk, Kab. Bogor, Jawa Barat 16740",
    "email": "warungmenteng83@gmail.com",
    "instagram": "@desa.warungmenteng",
    "instagram_url": "https://www.instagram.com/desa.warungmenteng/",
    "maps_url": "https://maps.google.com/?q=Desa+Warung+Menteng+Cijeruk+Bogor",
    "hours": "Senin \u2014 Jumat: 07:30 \u2014 16:30 WIB",
    "whatsapp": "08123456789",
    "whatsapp_url": "https://wa.me/628123456789"
  },
  "admin": {
    "username": "admin",
    "password": "warungmenteng",
    "title": "Panel Admin Desa Warung Menteng"
  },
  "footer_links": [
    { "title": "Navigasi", "links": [
      { "label": "Beranda", "href": "index.html" },
      { "label": "Tentang", "href": "tentang.html" },
      { "label": "Pariwisata", "href": "pariwisata.html" },
      { "label": "Galeri", "href": "galeri.html" },
      { "label": "KKN", "href": "kkn.html" }
    ]},
    { "title": "Informasi", "links": [
      { "label": "Berita", "href": "berita.html" },
      { "label": "Layanan", "href": "tentang.html#layanan" },
      { "label": "Struktur Desa", "href": "tentang.html#struktur" },
      { "label": "Anggaran", "href": "tentang.html#anggaran" }
    ]},
    { "title": "Lainnya", "links": [
      { "label": "Perangkat Desa", "href": "tentang.html#perangkat" },
      { "label": "Admin", "href": "admin.html" }
    ]}
  ]
};
